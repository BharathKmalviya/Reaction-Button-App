package com.example.samplecirclereactionlike.widget;
...
import com.example.samplecirclereactionlike.util.DisplayHelperKt;
...
public class ReactionView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int screenWidth = DisplayHelperKt.getScreenSize(getContext())[1];
        int desiredWidth = (screenWidth/90)*100;;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                onDeselect();
                if (selectedIndex!=-1){
                    Emotion  emotion= emotions.get(selectedIndex);
                    Toast.makeText(getContext(), emotion.title, Toast.LENGTH_SHORT).show();
                    if (mOnSelectReaction!=null){
                        mOnSelectReaction.onSelect(selectedIndex);
                    }
                }
                return true;
        }
    ...
    private OnSelectReaction mOnSelectReaction;

    public void setOnSelectReaction(OnSelectReaction mOnSelectReaction){
        this.mOnSelectReaction = mOnSelectReaction;
    }

    public interface OnSelectReaction{
    ...
    private void animateRoundedBoard(float interpolatedTime) {
        board.setCurrentHeight(board.startAnimatedHeight + (interpolatedTime * (board.endAnimatedHeight - board.startAnimatedHeight)));
    }
